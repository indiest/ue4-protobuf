...
FString FProtobufEditorModule::GetProtocPath()const
{
	...
	FString ProtocExe = FPaths::Combine(PluginPath, TEXT("Source/Protobuf/ThirdParty/bin/protoc.exe"));
...
FString FProtobufEditorModule::GetProtocIncludePath()const
{
	...
	FString PluginPath = IPluginManager::Get().FindPlugin(TEXT("Protobuf"))->GetBaseDir();
	PluginPath.Append(TEXT(""));
	FString ProtoInclude = FPaths::Combine(PluginPath, TEXT("Source/Protobuf/ThirdParty/"));
	return ProtoInclude;
}
...
void FProtobufEditorModule::ExecuteProtoc(const FString& pProtocExe, const FString& pProtoFilePath, FString pOutPath, FString pLang, bool pWaitProcExit)
{
	...
	for (const auto& ProtoFile : AllFiles)
	{
		...
		FString CommandParams = FString::Printf(TEXT("--proto_path=\"%s\" --proto_path=\"%s\" \"%s\" %s"), *local_ProtoPath, *GetProtocIncludePath(), *ProtoFile, *OutParams);
